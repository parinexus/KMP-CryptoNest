...
class CoinsRepositoryImplTest {
    ...
    @Test
    fun resolveMarketPrices_usesCacheWhenFresh() = runTest {
        val remote = FakeCoinsRemoteDataSource()
        val local = FakeMarketLocalDataSource()
        local.coins = listOf(TestCoins.bitcoinDto.toCoinInfoModel())
        local.coinsCachedAt = Clock.System.now().toEpochMilliseconds()
        val repository = FakeCoinsRepository(remote, local)

        val result = repository.resolveMarketPrices(listOf(TestCoins.BITCOIN_ID))

        assertThat(result).isInstanceOf(Result.Success::class)
        assertThat((result as Result.Success).data[TestCoins.BITCOIN_ID]).isEqualTo(50_000.0)
    }

    @Test
    fun resolveMarketPrices_servesCacheWhenRemoteFails() = runTest {
        val remote = FakeCoinsRemoteDataSource(
            coinsResult = Result.Error(DataError.Remote.NO_INTERNET),
        )
        val local = FakeMarketLocalDataSource()
        ...
        val result = repository.resolveMarketPrices(
            coinIds = listOf(TestCoins.BITCOIN_ID),
            forceRefresh = true,
        )
